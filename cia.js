...
window.renderCiaCard = (entry, options = {}) => {
  ...
  return `<div class="cia-card">${header}
    <section class="cia-section"><span class="cia-letter">C</span><div><p class="cia-section-name">Core · 核心</p><h4>${ciaEscape(cia.core.zh)}</h4>${cia.core.en ? `<p class="cia-english">${ciaEscape(cia.core.en)}</p>` : ""}</div></section>
    <section class="cia-section"><span class="cia-letter">I</span><div><p class="cia-section-name">Interlink · 构词与联想</p>${renderMorphology(cia.morphology)}</div></section>
    <section class="cia-section"><span class="cia-letter">A</span><div><p class="cia-section-name">Application · 应用</p>${renderApplication(entry)}</div></section>
  </div>`;
};
