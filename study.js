...
const renderVocabTheme = (theme) => {
  ...
  const showEntry = () => {
    ...
    lessonPanel.innerHTML = `<span class="tag">${index + 1} / ${theme.entries.length}</span>
      ...
      <ul class="details"><li><strong>常用搭配</strong><span lang="de">${entry.collocation}</span></li>
      <li><strong>原创例句</strong><span lang="de">${entry.example}</span><br><span class="muted">${entry.translation}</span></li></ul>
...
const renderGrammarTopic = (topic) => {
  ...
  lessonPanel.innerHTML = `<span class="tag">语法专题</span><h2>${topic.title}</h2>
    <p class="lead">${topic.rule}</p><div class="example-list">${topic.examples.map((text) => {
